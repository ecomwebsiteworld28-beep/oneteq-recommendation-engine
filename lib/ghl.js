...
const GHL_CUSTOM_FIELD_KEYS = {
  ...
  // AI-recommended one above). Now also read back by the staff page
  // itself, right after writing, to verify a save actually persisted
  // before reporting success to the browser — see GHL_CUSTOM_FIELD_IDS
  // below and handleSave in api/staff/[id].js.
  ...
  // (all five of the above are also in GHL_CUSTOM_FIELD_IDS now, for the
  // same save-verification reason)
  ...
  // set directly on the contact from the staff page's "Staff Assessment"
  // section. Created 2026-09-05 after confirming none of the four
  // existed yet anywhere in this GHL location.
  preferredTrainingStyle: 'preferred_training_style',
  individualAttentionPreference: 'individual_attention_preference',
  postnatalReturnToExercise: 'postnatal_return_to_exercise',
  clinicalBarrierToPrimaryGoal: 'clinical_barrier_to_primary_goal',
  // Section 21: "Professional overrides with mandatory reason" - this
  // field already existed, unused, when the earlier audit found it.
...
const GHL_CUSTOM_FIELD_IDS = {
  ...
  // Added 2026-09-05 so the staff page can verify its own save actually
  // persisted (by re-checking these against what PUT echoes back) before
  // ever telling the browser "Saved to GHL."
  finalPackageSummary: 'EyWKtKGWsW2p4wHNIyut',
  finalPriceMonthly: 'emXptvuDrV75j22FHL19',
  finalPriceOneOff: '5MbxIF8s5BzjpAX4MhOj',
  finalPackageItemKeys: 'DnQMykFwwGatIjWLht9m',
  finalPackageDecisions: 'wgdkBlI5TGFE5faNBqvi',
  // Brief section 21's four staff-only inputs, created 2026-09-05.
  preferredTrainingStyle: 'vRvc0M3tLftkeSXi8Svm',
  individualAttentionPreference: 'gYZYjFTD7vPqzZNeR77x',
  postnatalReturnToExercise: 'X8VbqGo8PTzA9ATuasQN',
  clinicalBarrierToPrimaryGoal: 'A1Gvzs2Ldy3lUMoAdXYc',
  // Pre-existing, previously unused - see GHL_CUSTOM_FIELD_KEYS.overrideReason.
  overrideReason: '4T1hewcbzL3fb3ZVQfaJ',
};
...
const GHL_SURVEY_ANSWER_FIELD_IDS = {
  ...
  // Added 2026-09-05 after the full input audit found these collected but
  // never read: Q17_Event_Detail (free text, e.g. "training for a half
  // marathon") and Balanced_Training_Need (a plain Yes/No checkbox).
  q17Detail: 'XkW1IsLaPOhpNs4M7Kv9', // Q17_Event_Detail
  balancedTrainingNeed: 'eTjuaIPcwsNK3tuT4vC3', // Balanced_Training_Need
};
...
// Returns the parsed response body (GHL echoes the full updated contact
// back as {contact: {...}}) rather than void — a 2xx status alone doesn't
// prove a given field actually took the new value, so a caller that needs
// to be sure (e.g. the staff page, before reporting "Saved to GHL") can
// check the echoed contact's customFields itself instead of trusting the
// HTTP status in isolation.
